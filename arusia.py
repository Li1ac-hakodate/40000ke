...
async def on_message(message):
    ...
    if content.startswith("허경영"):
        embed = discord.Embed(title="롸잇 나우", description="너는 갠디로 도움말을 받았을 것이야", color=0x00aaaa)
        await message.channel.send(embed=embed)  
        ...
        embed.add_field(name=">개발자소개", value=' - 제작자는 대체 뭐하는 새낀가', inline=False)
        ...
        embed.add_field(name=">SuppotServer", value=' - Suppot Server 링크를 가져옵니다.', inline=False)
    ...
    if content == ">개발자소개": 
        ...
        await message.channel.send(embed=embed)
    if content == ">문의": 
        embed = discord.Embed(title="모든 의견을 공중부양 시키겠습니다. 언제든지 Аshiуаlice#4462로 항상 연락하십시오!", color=0x00aaaa)
        await message.channel.send(embed=embed)
    if content == ">愛は嫌いだ": 
    ...
    if content == ">k_help": 
        ...
        embed.add_field(name=">19사이트모음", value=' - **반드시 nsfw 방에서만 사용하십시오!!**', inline=False)
        embed.add_field(name=">실시간검색어", value=' - 네이버의 현재 실검을 불러옵니다.', inline=False)
        embed.add_field(name=">SuppotServer", value=' - 허경영 봇 커뮤니티 링크를 가져옵니다.', inline=False)
        await message.channel.send(embed=embed)
    if content == ">J_help": 
